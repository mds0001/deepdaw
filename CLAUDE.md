...
- **`MainComponent`** owns the `juce::AudioDeviceManager` (initialised with 2-in/2-out defaults) and lays out children in `resized()` using JUCE's `removeFromTop`/`removeFromLeft` rectangle-slicing idiom: transport bar (top 48 px), a controls strip with the `+ Audio/MIDI Track` buttons (36 px), then a left track-list pane (`trackListWidth = 280`) and a right timeline pane. Both panes live in viewports; its nested `SyncViewport` reports vertical scroll so the two stay row-aligned (guarded by `syncingScroll` against feedback). Its destructor calls `setLookAndFeel(nullptr)`.
...
Phase 1 components (built and live as of commit `ca79605`):
...
- **`TimelineComponent`** holds a `TrackListComponent&` and draws the ruler + per-track lanes aligned to the list rows (`rowHeight = 64`, `rulerHeight = 32`, `pixelsPerBar = 80`, `numBars = 64`). `updateContentSize()` sizes it to fit the track count so its host viewport gets the right scroll extents — call it after tracks change.

Still unimplemented from the Phase 1 spec (`docs/PHASE1_*`): playhead + transport sync, click-to-set-position, horizontal zoom, and persisting the track list in the `.deepdaw` file.
...
- **New `.cpp` files must be added to `target_sources(DeepDAW ...)` in `CMakeLists.txt`** or they won't compile/link — only `.cpp` files are listed there, not headers. A source on disk that isn't listed silently never builds.
