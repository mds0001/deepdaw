    ...
    : track(t), displayIndex(index)
{
    ...
    panSlider.setDoubleClickReturnValue(true, 0.0); // double-click to centre
    ...
    {
        double v = panSlider.getValue();
        if (std::abs(v) < 0.05) // soft detent at centre
        {
            v = 0.0;
            panSlider.setValue(0.0, juce::dontSendNotification);
        }
        track.pan = (float) v;
        repaint(); // refresh the PAN value caption
        if (onMixChanged) onMixChanged();
    };
    addAndMakeVisible(panSlider);

    // Fader is calibrated in dB (-inf..+6, unity at 0 dB); the model stores the
    // equivalent linear gain.
    gainFader.setSliderStyle(juce::Slider::LinearVertical);
    gainFader.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
    gainFader.setRange(-60.0, 6.0, 0.1);
    gainFader.setDoubleClickReturnValue(true, 0.0); // 0 dB
    gainFader.setValue(juce::Decibels::gainToDecibels((double) track.gain, -60.0), juce::dontSendNotification);
    ...
    {
        track.gain = (float) juce::Decibels::decibelsToGain(gainFader.getValue(), -60.0);
...
void ChannelStripComponent::refreshMeter()
{
    ...
    meterHold  = juce::jmax(level, meterHold - 0.010f);  // slow-decaying peak-hold
    repaint(meterBounds);
}
...
void ChannelStripComponent::paint(juce::Graphics& g)
{
    ...
    // "PAN" caption + current pan value (L/C/R).
    auto capRow = panSlider.getBounds().translated(0, -13).withHeight(12);
    g.setColour(juce::Colour(0xff8a8a8a));
    g.setFont(11.0f);
    g.drawText("PAN", capRow, juce::Justification::centredLeft, false);
    const int panPct = juce::roundToInt(std::abs(track.pan) * 100.0f);
    juce::String panText = track.pan == 0.0f ? "C"
                         : (track.pan < 0.0f ? "L" + juce::String(panPct)
                                             : "R" + juce::String(panPct));
    g.setColour(juce::Colour(0xffb0b0b0));
    g.drawText(panText, capRow, juce::Justification::centredRight, false);

    // dB readout under the fader.
    if (! readoutBounds.isEmpty())
    {
        const double dB = juce::Decibels::gainToDecibels((double) track.gain, -60.0);
        juce::String dbText = dB <= -60.0 ? "-inf"
                            : (dB >= 0.0 ? "+" : "") + juce::String(dB, 1);
        g.setColour(juce::Colour(0xffb0b0b0));
        g.setFont(11.0f);
        g.drawText(dbText, readoutBounds, juce::Justification::centred);
    }

    // Output level meter (post-fader/pan), bottom-up fill, with a peak-hold tick.
    if (! meterBounds.isEmpty())
    {
        ...
        const float hold = juce::jlimit(0.0f, 1.0f, meterHold);
        if (hold > 0.0f)
        {
            const int ty = meterBounds.getBottom() - juce::roundToInt(meterBounds.getHeight() * hold);
            g.setColour(hold > 0.9f ? juce::Colour(0xffff1744) : juce::Colour(0xffd0d0d0));
            g.fillRect(meterBounds.getX(), ty - 1, meterBounds.getWidth(), 2);
        }
...
void ChannelStripComponent::resized()
{
    ...
    readoutBounds = area.removeFromBottom(14); // dB value
    area.removeFromBottom(4);
